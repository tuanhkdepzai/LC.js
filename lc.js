...
const axios = require('axios'); // ĐÃ SỬA: Thêm axios
...
class UltraDicePredictionSystem {
    ...
    // ĐÃ SỬA: Hàm đồng bộ mảng dữ liệu lịch sử từ API một cách mượt mà
    ...
    initPatternDatabase() {
        this.patternDatabase = {
            ...
            // BỔ SUNG: Toàn bộ cấu trúc các loại cầu phổ biến khác
    ...
    initAdvancedPatterns() {
        this.advancedPatterns = {
            'dynamic-1': {
                ...
                predict: () => 'X', confidence: 0.72, description: "4T trong 6 phiên, cuối là T -> dự đoán X"
            },
            'dynamic-2': {
                ...
                predict: () => 'X', confidence: 0.78, description: "6+T trong 8 phiên, cuối là T -> dự đoán X mạnh"
            },
            'alternating-3': {
                ...
                predict: (data) => data[data.length-1] === 'T' ? 'X' : 'T', confidence: 0.68, description: "5 phiên đan xen (Cầu 1-1) -> tiếp tục bám dải hoặc chờ bẻ"
            },
            'cyclic-7': {
                detect: (data) => { if (data.length < 14) return false; return this.arraysEqual(data.slice(-14, -7), data.slice(-7)); },
                predict: (data) => data[data.length-7], confidence: 0.75, description: "Chu kỳ lặp đối xứng 7 phiên"
            }
        };
    }
    ...
    addResult(result) {
        ...
        } else { this.sessionStats.streaks[result] = 1; }
    ...
    model1() {
        ...
        return { prediction: bestPattern.prediction, confidence: Math.min(0.95, bestPattern.probability * 0.8), reason: `Khớp chuỗi Pattern ${bestPattern.type}` };
    }
    model1Mini(data) {
        const patterns = [];
    ...
    model2() {
        ...
        return { prediction, confidence: 0.65, reason: `Quét xu hướng thị trường` };
    }
    ...
    model3() {
        ...
        if (Math.abs(tCount - (12 - tCount)) / 12 > 0.4) {
            return { prediction: tCount > 6 ? 'X' : 'T', confidence: 0.7, reason: "Hồi tụ cân bằng Mean Reversion" };
        }
        return null;
    }
    model3Mini() { return {}; } model3Support1() { return {}; } model3Support2() { return {}; } model3Support3() { return {}; } model3Support4() { return {}; }

    // Thêm stubs cho toàn bộ các model từ 4 đến 21 để tránh lỗi hệ thống bị rỗng
    model4() { if (this.history.length < 3) return null; return { prediction: this.history[this.history.length-1], confidence: 0.55, reason: "Momentum ngắn" }; }
    ...
    model18() { for (const [pName, pConfig] of Object.entries(this.advancedPatterns)) { if (pConfig.detect(this.history)) return { prediction: pConfig.predict(this.history), confidence: pConfig.confidence, reason: `Cầu nâng cao: ${pConfig.description}` }; } return null; }
    ...
    // ĐÃ SỬA: Tên hàm chuẩn hóa khớp với logic điều phối API ở dưới
    getFinalPrediction() {
        ...
        for (const [mName, pData] of Object.entries(predictions)) {
            ...
            if (pData.prediction === 'T') { totalTWeight += pData.confidence * w; countT++; }
            else if (pData.prediction === 'X') { totalXWeight += pData.confidence * w; countX++; }
            explanations.push(`[${mName}]: ${pData.prediction} (${Math.round(pData.confidence*100)}%)`);
        }

        const totalWeightSum = totalTWeight + totalXWeight;
        if (totalWeightSum === 0) { return { du_doan: "?", ty_le_thanh_cong: "0%", giai_thich: "Hệ thống đang tích lũy dữ liệu chuỗi." }; }
        ...
        return {
            ...
            giai_thich: explanations.slice(0, 2).join(' | ') || "Tổng hợp ma trận trọng số."
        };
    }
}
...
// ĐÃ SỬA: Chỉ khai báo 1 biến duy nhất toàn cục bằng let
let apiResponseData = {
    ...
    giai_thich: "Đang khởi tạo...", pattern: "N/A"
};
...
function fetchDataFromNewAPI() {
    ...
    .then(response => {
        ...
        if (data && data.list && Array.isArray(data.list) && data.list.length > 0) {
            ...
            // ĐÃ SỬA: Gọi đúng hàm updateHistory đã viết trong class
            ...
            if (t8.match) {
                ...
                giaiThichFinal = `Khợp chuỗi khuôn cứng mục tiêu [${t8.pattern}] từ Database Thuật toán 8.`;
            } else {
                // ĐÃ SỬA: Gọi đúng hàm getFinalPrediction thực tế
...
// ĐÃ SỬA: Chỉnh sửa chuỗi Template Literal hiển thị chuẩn giao diện web
app.get('/', (req, res) => {
    res.send(`
        <h2>🎯 Hệ thống Matrix Engine AI Sunwin v2026</h2>
        <p><b>Kết quả phiên trước:</b> Phiên ${apiResponseData.phien_vua_ra || '...'} -> Tổng: ${apiResponseData.tong || '...'} (${apiResponseData.ket_qua_vua_ra || '...'})</p>
        <p><b>DỰ ĐOÁN PHIÊN KẾ TIẾP (${apiResponseData.phien || '...'}):</b> <span style="color:cyan; background:black; padding:3px 8px; font-weight:bold;">${apiResponseData.du_doan}</span> (${apiResponseData.ty_le_thanh_cong})</p>
        <p><b>Chi tiết cấu trúc thuật toán phát hiện:</b> ${apiResponseData.giai_thich}</p>
        <p><b>Chuỗi quét vết:</b> ${apiResponseData.pattern}</p>
        <hr/>
        <p><a href="/api/ditmemaysun" target="_blank">Xem cổng JSON Realtime</a> | <a href="/api/prediction-status" target="_blank">Xem trạng thái mạng 21 Models</a></p>
    `);
});
